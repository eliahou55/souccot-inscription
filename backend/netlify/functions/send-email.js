...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background:#fff;">
  <tr><td style="background:#1a6b2f;padding:25px;text-align:center;">
    <h1 style="color:#fff;margin:0;font-size:22px;">LOISIREL SOUCCOT 2026</h1>
    <p style="color:#d7f5da;margin:5px 0 0;">Hotel Millenium Charles de Gaulle</p>
  </td></tr>
  <tr><td style="padding:30px;">
    <h2 style="color:#1a6b2f;margin-top:0;">Confirmation d'inscription</h2>
    ...
    <table width="100%" cellpadding="8" cellspacing="0" style="border-collapse:collapse;font-size:14px;">
      <tr style="background:#eef9ec;"><td style="border:1px solid #ddd;"><strong>Contact</strong></td><td style="border:1px solid #ddd;">${data.prenomContact} ${data.nomContact} — ${data.portable || 'N/A'}</td></tr>
      <tr><td style="border:1px solid #ddd;"><strong>Personnes</strong></td><td style="border:1px solid #ddd;">${data.nombrePersonnes}</td></tr>
      ${remiseLine}
      <tr style="background:#1a6b2f;"><td style="border:1px solid #1a6b2f;color:#fff;padding:10px;"><strong>TOTAL</strong></td><td style="border:1px solid #1a6b2f;color:#fff;padding:10px;"><strong>${finalTotal}€</strong></td></tr>
      <tr><td style="border:1px solid #ddd;">Acompte 50%</td><td style="border:1px solid #ddd;">${data.acomptEUR}€</td></tr>
      <tr style="background:#eef9ec;"><td style="border:1px solid #ddd;">Solde</td><td style="border:1px solid #ddd;">${data.soldeEUR}€</td></tr>
    </table>
    <div style="background:#fff8e1;border-left:4px solid #f59e0b;padding:15px;margin-top:20px;border-radius:4px;">
      ...
      <strong style="color:#1a6b2f;">Titulaire du compte : TOVEL</strong><br>
  ...
  <tr><td style="background:#eef9ec;padding:15px;text-align:center;font-size:12px;color:#888;">
...
function buildOrganizerHTML(data, finalTotal) {
    ...
    const membresRows = membres.map((m, i) =>
        `<tr style="${i % 2 === 0 ? 'background:#eef9ec;' : ''}"><td style="border:1px solid #ddd;padding:6px;">${m.nom} ${m.prenom}</td><td style="border:1px solid #ddd;padding:6px;">${CAT_LABELS[m.categorie] || m.categorie || 'N/A'}</td></tr>`
    ).join('');
    const remiseLine = (data.remiseAppliquee === 'Oui' && data.remiseAmount > 0)
        ? `<tr style="background:#eef9ec;"><td style="border:1px solid #ddd;"><strong>Total avant remise</strong></td><td style="border:1px solid #ddd;">${data.totalEUR}€</td></tr>` : '';
    return `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"></head>
<body style="margin:0;padding:0;font-family:Arial,sans-serif;background:#f4f4f4;">
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background:#fff;">
  <tr><td style="background:#1a6b2f;padding:20px;text-align:center;">
    <h1 style="color:#fff;margin:0;font-size:18px;">NOUVELLE INSCRIPTION — SOUCCOT 2026</h1>
  </td></tr>
  <tr><td style="padding:25px;">
    <h2 style="color:#1a6b2f;margin-top:0;">${data.prenomContact} ${data.nomContact}</h2>
    <table width="100%" cellpadding="7" cellspacing="0" style="border-collapse:collapse;font-size:14px;">
      <tr style="background:#eef9ec;"><td style="border:1px solid #ddd;width:40%;"><strong>Telephone</strong></td><td style="border:1px solid #ddd;">${data.portable || 'N/A'}</td></tr>
      <tr><td style="border:1px solid #ddd;"><strong>Email</strong></td><td style="border:1px solid #ddd;">${data.emailContact}</td></tr>
      <tr style="background:#eef9ec;"><td style="border:1px solid #ddd;"><strong>Personnes</strong></td><td style="border:1px solid #ddd;">${data.nombrePersonnes} (${data.chambresAdultes} adultes, ${data.chambresEnfants} enfants, ${data.bebes} bebes)</td></tr>
    ...
    ${membres.length > 0 ? `<h3 style="color:#1a6b2f;margin-top:20px;">Membres de la famille</h3>
    <table width="100%" cellpadding="6" cellspacing="0" style="border-collapse:collapse;font-size:13px;">
      <tr style="background:#1a6b2f;color:#fff;"><th style="padding:8px;text-align:left;">Nom Prenom</th><th style="padding:8px;text-align:left;">Categorie</th></tr>
      ${membresRows}
    </table>` : ''}
